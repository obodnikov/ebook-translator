...
### Итерация 6 — Pause-points + Telegram

**Цель.** Длинный прогон не должен требовать сидеть у экрана.

- `notifier.TelegramNotifier` (httpx).
- Pause-point 1: после glossary (состояние записано, Telegram → ждём
  ручной правки, `--resume` продолжает).
- Pause-point 2: после translate, до вычитки — Telegram с
  `translated-preview.epub`.
- `state.json`, `pipeline.Stage`, логика `--resume`.
- `.env`: `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`.

### Итерация 7 — Оптимизация: relevancy-фильтр глоссария
