...
- Схема:
  - Добавить в `GlossaryEntry` / `SeriesGlossaryEntry` поле
    `reader_note: str | None`. Это отдельный регистр от `notes`
    (переводческих заметок): короткое пояснение для читателя на
    {target_lang}. Пустое поле = сноска не ставится.
  - Опциональное поле `comment: bool = False` или политика по `type`
    (например, по умолчанию комментировать только `concept` и `term`).
- Механизм постановки сносок — вариант A (предпочтительный):
  при переводе chunk-а подавать модели список терминов-кандидатов с их
  `reader_note`, и инструкцию "при первом упоминании в этом chunk-е
  оберни термин в EPUB footnote-ref с данным содержимым". Модель сама
  обрабатывает русскую морфологию и не ставит сноски в ненужных местах.
- Альтернатива — вариант B (пост-обработка): искать термины в готовом
  русском тексте через `pymorphy3` + leven по основе. Хрупче и дороже.
- Контроль читательской нагрузки:
  - По умолчанию сноска только на ПЕРВОЕ упоминание в главе.
  - По умолчанию комментируем ТОЛЬКО тип `concept` (магические
    заклинания, инвентарь мира, ритуалы — то, что русскому читателю
    непонятно в принципе). Остальные типы (`person`, `place`, `term`,
    `other`) по умолчанию не комментируются, даже если `reader_note`
    заполнен — но его можно включить явно.
  - Флаг конфига `reader_notes.scope: first-in-chapter | first-in-book
    | all`.
  - Флаг конфига `reader_notes.types: [concept]` — какие типы терминов
    комментировать. Расширить до `[concept, term]` если нужно пояснять
    полицейский жаргон (DCI, CID, TSG) или до `[concept, term, place]`
    если важно объяснять важные локации (the Folly).
  - Флаг в отдельной записи `comment_override: "always" | "never" |
    null` — ручное управление для единичных случаев.
- Источник `reader_note`:
  - Вручную в series-glossary (переводчик пишет).
  - Сгенерировать отдельным промптом `notes_generate.md` на основе
    `notes` + исходного контекста. Человек проверяет и правит.
