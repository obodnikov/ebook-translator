...
- Обновление метаданных: `dc:language = ru`, `dc:title` = переведённое название (из glossary или спросить у LLM отдельным вызовом), `dc:creator` как был (транслитерация опционально). `dc:title` и `dc:creator` записывает шаг обложки, когда ему переданы `--cover-title` / `--cover-author`. Туда же относится объявление обложки — см. `cover.py` ниже.
...
def repair_cover(source_epub, dest_epub, *, title, author) -> tuple[CoverInfo, CoverFixes]:
    # Починка описания обложки без обращения к модели: объявление обложки,
    # размер на странице обложки, dc:title / dc:creator. Картинку не трогает.
```
...
- **Fix** (`btrans cover fix`): починка уже собранной книги, без вызова модели.

**Почему обложку приходится объявлять заново.** Во многих исходных книгах (в
частности от OceanofPDF) картинка обложки лежит в списке файлов, но нигде не
сказано, что она — обложка: нет ни `<meta name="cover">`, ни `<guide>` со
ссылкой `type="cover"`. Читалка не может её найти и рисует свою заглушку из
`dc:title` и `dc:creator` — на полке видно английское название при русской
обложке внутри. Поэтому запись обложки (`replace`, `translate`, `fix`)
дописывает объявление, если его нет: для EPUB2 — `<meta name="cover">` и
ссылку в `<guide>`, для EPUB3 — `properties="cover-image"` у элемента
манифеста.

Там же правится размер, объявленный страницей обложки: `viewBox` и размеры
`<image>` в свёрстанной calibre странице описывают прежнюю картинку, и при
других пропорциях новая обложка вписывается в старую рамку с пустыми полосами.
Размер читается прямо из заголовка файла картинки; если прочитать не удалось,
разметка остаётся прежней — лучше устаревший размер, чем неверный.
...
**v1.3** (reader notes from glossary, **реализовано**):
- Используем глоссарий как источник подстрочных читательских сносок
  в финальном EPUB. Никаких новых полей в модели — используем
  существующие `type` + `notes` + `translation`.
- Механизм — детерминистическая пост-обработка при сборке:
  - На этапе `btrans assemble --notes` или как часть `btrans translate`
    (включено по умолчанию при наличии глоссария): фильтруем glossary по
  ...
  - Первое вхождение (по scope) помечаем номером сноски, `notes` идёт в
    конец главы.
- **Разметка выбирается по версии пакета исходной книги** (`BookMeta.epub_version`).
  Всплывающие сноски — механизм EPUB 3; в EPUB 2 его нет, и `<aside>` там даже
  не разрешён — строгая читалка вправе выбросить элемент вместе с текстом
  сноски, и ссылка ведёт в никуда. Поэтому:
  - EPUB 3 → `<aside epub:type="footnote">`, читалка открывает окошко;
  - EPUB 2 → блок «Примечания» в конце главы из обычных `<div>`, которые
    сохраняет любая читалка; прятать его нельзя, иначе нажатие на номер
    приводит в пустоту.
  В обоих случаях у маркера есть свой якорь (`reader-note-N-ref`), а у
  примечания — обратная ссылка на него.
- Атрибут `epub:type` создаётся с привязкой префикса `epub` к пространству имён:
  без этого lxml придумывает свой (`ns0:type`) в книгах, чей `<html>` не
  объявляет `xmlns:epub` — формально верно, но читалки, сверяющие буквальный
  `epub:type`, такую сноску не видят.
- Контроль читательской нагрузки:
  ...
  - `reader_notes.enabled: true` по умолчанию; `--notes` / `--no-notes` в CLI.
- `btrans translate` вставляет сноски автоматически (after rehydrate,
  before write); `btrans assemble --notes` остаётся для ручной сборки.
